...
class TelephoneCascadeBuilder:
    ...
    def build(
            ...
            *,
            ...
            """
            Build a branching "telephone tree" dataset.

            Key behaviors (as requested):
              - Each node (root + every hop) is written as its own .txt file.
              - Cascading summaries: each hop summarizes the previous hop (parent) text.
              - Dataset size is determined by:
                    (#roots) × (1 + k + k^2 + ... + k^depth)
                (plus optional control-condition outputs, if enabled).
            """
            cfg = self.cfg
            out_dir.mkdir(parents=True, exist_ok=True)

            # Telephone mode always writes .txt files per node (required for hop-wise datasets).
            cfg.export_txt_nodes = True

            if not cfg.export_jsonl and not cfg.export_txt_nodes:
            ...
            root_meta_map: Dict[str, Dict[str, str]] = {rid: meta for rid, _t, meta in roots}

            n_roots = len(roots)
            k = max(0, int(cfg.branching_factor))
            d = max(1, int(cfg.depth))

            # Tree size per root: 1 + k + ... + k^d
            if k == 1:
                per_root = d + 1
            else:
                per_root = 1 + sum(k ** i for i in range(1, d + 1))

            expected_cascade_total = n_roots * per_root  # includes roots
            expected_control_total = 0
            if bool(cfg.include_control):
                expected_control_total = n_roots * max(0, int(cfg.control_children_per_hop)) * d

            expected_total = expected_cascade_total + expected_control_total

            # Safety guard against accidental explosion
            if expected_total > 5_000_000:
                raise ValueError(
                    f"Telephone mode would generate {expected_total:,} samples (.txt files), which is too large for safety.\n\n"
                    f"Reduce roots / depth / branching factor, or edit the safety limit in code."
                )

            self.log(
                f"Telephone tree mode: roots={n_roots} depth={d} k={k} "
                f"expected_txt={expected_cascade_total:,}"
                + (f" (+control={expected_control_total:,})" if expected_control_total else "")
            )
            ...
            pad = max(0, int(zero_pad_width))
            next_txt_index = int(start_index)
            created_txt_files: List[str] = []
            total_written = 0

            def _mk_txt_name(idx: int) -> str:
                ...
                if not ext.startswith("."):
                    ext = "." + ext
                return f"{output_prefix}{num}{output_suffix}{ext}"

            # Open JSONL file (or devnull if disabled)
            jsonl_handle = (
                jsonl_path.open("w", encoding="utf-8")
                if (jsonl_path is not None and cfg.export_jsonl)
            ...
            def _fallback_summary(parent_text: str, target_chars: int, slack_chars: int) -> str:
                """Deterministic fallback that guarantees a non-empty string within the length range."""
                parent_text = _normalize_ws(parent_text)
                if not parent_text:
                    return ""
                lo = max(0, int(target_chars) - int(slack_chars))
                hi = max(lo, int(target_chars) + int(slack_chars))

                # Prefer a prefix of the parent; cut to word boundary.
                take = min(max(lo, min(int(target_chars), len(parent_text))), len(parent_text))
                cand = parent_text[:take].strip()

                if len(cand) > hi and hi > 0:
                    cand = cand[:hi].rsplit(" ", 1)[0].strip() if " " in cand else cand[:hi].strip()

                if len(cand) < lo:
                    # If still short, take a slightly longer prefix if possible.
                    take2 = min(len(parent_text), hi)
                    cand = parent_text[:take2].strip()
                    if len(cand) > hi and hi > 0:
                        cand = cand[:hi].rsplit(" ", 1)[0].strip() if " " in cand else cand[:hi].strip()

                return _normalize_ws(cand)

            def _generate_child_strict(parent_text: str, hop: int, target_chars: int, slack_chars: int) -> Dict[str, object]:
                """
                Generate ONE child summary from `parent_text`, attempting to satisfy:
                  - length window target±slack
                  - semantic similarity band (S_min..S_max)
                  - lexical overlap band (L_min..L_max)
                  - n-gram copy ratio <= max
                  - dedupe threshold

                To keep dataset size deterministic, if no candidate satisfies all constraints after
                `max_attempts_per_child`, this will "force-accept" the best-scoring candidate (or a
                deterministic fallback) and mark it in metadata.
                """
                ...
                max_copy = float(cfg.max_ngram_copy_ratio)
                dedupe_thr = float(cfg.dedupe_threshold)
                ...
                for attempt in range(1, attempts + 1):
                    ...
                    # Enforce length window (like MDIC's target±slack constraint).
                    if len(cand) < lo or len(cand) > hi:
                        self.rejected["length"] += 1
                        continue

                    ok_dedupe, best_dedupe = self._dedupe_ok(cand)
                    sim = self.scorer.similarity(parent_text, cand)
                    lex = _lexical_overlap_jaccard(parent_text, cand)
                    copyr = _ngram_copy_ratio(parent_text, cand, n=4)

                    in_s = (s_min <= sim <= s_max)
                    in_l = (l_min <= lex <= l_max)
                    ok_copy = (copyr <= max_copy)

                    passes = bool(ok_dedupe and in_s and in_l and ok_copy)

                    pen = (
                        _band_penalty(sim, s_min, s_max)
                        + _band_penalty(lex, l_min, l_max)
                        + max(0.0, float(copyr) - max_copy) * 2.0
                    )
                    if not ok_dedupe and dedupe_thr < 100.0:
                        pen += max(0.0, float(best_dedupe) - dedupe_thr) / 100.0

                    cand_info: Dict[str, object] = {
                        "text": cand,
                        "attempt": attempt,
                        "forced_accept": False,
                        "passes_all_gates": passes,
                        "gates": {
                            "semantic_in_band": in_s,
                            "lexical_in_band": in_l,
                            "copy_ok": ok_copy,
                            "dedupe_ok": bool(ok_dedupe),
                        },
                        "metrics": {
                            "char_len": len(cand),
                            ...
                            "dedupe_score_max": float(best_dedupe),
                        },
                        "penalty": float(pen),
                    }
                ...
                # Force-accept the best candidate, or a deterministic fallback.
                if best is None:
                    fb = _fallback_summary(parent_text, target, slack)
                    if not fb:
                        fb = parent_text[:max(1, min(len(parent_text), max(1, target)))].strip()

                    ok_dedupe, best_dedupe = self._dedupe_ok(fb)
                    sim = self.scorer.similarity(parent_text, fb)
                    lex = _lexical_overlap_jaccard(parent_text, fb)
                    copyr = _ngram_copy_ratio(parent_text, fb, n=4)
                    ...
                    best = {
                        ...
                        "gates": {
                            "semantic_in_band": bool(s_min <= sim <= s_max),
                            "lexical_in_band": bool(l_min <= lex <= l_max),
                            "copy_ok": bool(copyr <= max_copy),
                            "dedupe_ok": bool(ok_dedupe),
                        },
                        "metrics": {
                            "char_len": len(fb),
                    ...
                    best["forced_accept"] = True

                self.rejected[f"forced_accept_h{hop}"] += 1
                return best

            with jsonl_handle as f_jsonl:

                def _emit(rec: Dict[str, object], text: str) -> None:
                    nonlocal total_written, next_txt_index

                    # Always write TXT node
                    fname = _mk_txt_name(next_txt_index)
                    next_txt_index += 1
                    path = out_dir / fname
                    ...
                    created_txt_files.append(fname)
                    rec["txt_file"] = fname

                    # Optional JSONL
                    if cfg.export_jsonl and jsonl_path is not None:
                ...
                for root_id, root_text, meta in roots:
                    # Root node id stable: sample_id == root_id
                    rec = {
                        "sample_id": root_id,
                        "root_id": root_id,
                        "parent_id": None,
                ...
                # ------------------------------ Control ------------------------------
                if bool(cfg.include_control) and int(cfg.control_children_per_hop) > 0:
                    ...
                    for root_id, root_text, meta in roots:
                        for hop in range(1, d + 1):
                            ...
                            for _ in range(int(cfg.control_children_per_hop)):
                                child_info = _generate_child_strict(root_text, hop, target, slack)
                                child = str(child_info["text"])
                                ...
                                rec = {
                                    ...
                                    "slack_chars": slack,
                                    "text": child,
                                    "metrics": child_info["metrics"],
                                    "gates": child_info.get("gates", {}),
                                    "forced_accept": bool(child_info.get("forced_accept", False)),
                                    "generation": {
                                        "backend": "extractive_v1",
                                        "attempt": int(child_info.get("attempt", 0) or 0),
                                    },
                                    "source": meta,
                                }
                ...
                while frontier:
                    ...
                    # IMPORTANT: "telephone game" behavior — each hop uses the last hop's text as the new seed.
                    # Enforce EXACT branching factor: k children per node.
                    for _child_i in range(k):
                        child_info = _generate_child_strict(parent_text, next_hop, target, slack)
                        child = str(child_info["text"])
                        ...
                        rec = {
                            ...
                            "source": root_meta_map.get(root_id, {}),
                        }
            ...
            if export_settings_json:
                ...
                payload: Dict[str, object] = {
                    ...
                    "output": {
                        ...
                        "export_txt_nodes": True,
                    },
                    "expected_counts": {
                        "roots": n_roots,
                        "depth": d,
                        "branching_factor": k,
                        "cascade_total_txt": int(expected_cascade_total),
                        "control_total_txt": int(expected_control_total),
                        "total_txt": int(expected_total),
                    },
                    "stats": {
            ...
            # Sanity check: in cascade-only mode, this should match expected_cascade_total
            # (Control adds additional samples if enabled).
            if total_written != expected_total:
                self.log(
                    f"[Warning] Wrote {total_written:,} sample(s), expected {expected_total:,}. "
                    "This usually indicates unexpected early termination."
                )
            ...
            return {
                ...
                "total_samples": total_written,
                "expected_total_txt": int(expected_total),
                "rejected": dict(self.rejected),
            }
...
class App(ttk.Frame):
    ...
    def _build_ui(self) -> None:
        ...
        self.cb_tel_control = tk.BooleanVar(value=False)
        ...
        self.e_tel_max_samples = LabeledEntry(self.g_tel_branch, "Max total samples (legacy cap; ignored)", width=10)
        ...
        # Telephone tree mode always writes a .txt file per node (required for hop-wise datasets).
        self.cb_tel_txt = tk.BooleanVar(value=True)
        self.w_tel_txt = ttk.Checkbutton(
            self.g_tel_export,
            text="Export .txt files for each node (required)",
            variable=self.cb_tel_txt,
            command=self._update_preview,
        )
        self.w_tel_txt.grid(row=2, column=0, sticky="w", pady=(6, 4))
        self.w_tel_txt.configure(state="disabled")
    ...
    def _read_tel_cfg_from_ui(self) -> TelephoneConfig:
        ...
        # Telephone tree mode always writes .txt nodes (one per hop/node).
        t.export_txt_nodes = True
